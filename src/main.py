...
"""
    ...
    def open_camera_view(self):
        ...
        if self.camera_view_window is None:
            self.camera_view_window = CameraViewWindow(self)
    ...
    def check_microscopy_position(self):
        ...
        try:
            ...
            if error_x <= tolerance and error_y <= tolerance:
                ...
                # Esperar estabilización (500ms) y capturar
                QTimer.singleShot(500, self.capture_microscopy_image)
            else:
    ...
    def capture_microscopy_image(self):
        ...
        try:
            ...
            # Procesar canales (si la imagen es grayscale, convertir a BGR primero)
            if len(frame.shape) == 2:  # Grayscale
                # Convertir a BGR para poder aplicar filtros de canales
                frame = cv2.cvtColor(frame, cv2.COLOR_GRAY2BGR)
            
            if len(frame.shape) == 3:  # Color o convertida a color
                ...
                if len(channels_to_keep) < 3:
                    # Crear imagen con solo los canales seleccionados
            ...
            # Pequeña pausa antes del siguiente punto (500ms)
            QTimer.singleShot(500, self.execute_microscopy_point)
    ...
    # --- Generador de Trayectorias en Zig-Zag ---
    
    def generate_zigzag_trajectory(self):
        """Genera una trayectoria en zig-zag con puntos homogéneos en distancia."""
        ...
        try:
            ...
            logger.info(f"Generando trayectoria: {n_points} puntos")
            logger.info(f"  X: {x_start} → {x_end} µm (medio: {x_mid:.1f})")
            logger.info(f"  Y: {y_start} → {y_end} µm (medio: {y_mid:.1f})")
            logger.info(f"  Delay: {step_delay}s entre puntos")
            ...
            self.step_trajectory_status.setText(f"✅ Trayectoria lista: {len(trajectory)} puntos")
            ...
            self.microscopy_trajectory_status.setText(f"✅ Trayectoria lista: {len(trajectory)} puntos")
            ...
            logger.info(f"✅ Trayectoria generada: {len(trajectory)} puntos")
            QMessageBox.information(self, "✅ Trayectoria Generada", 
                                   f"Trayectoria en zig-zag generada exitosamente:\n\n"
                                   f"Puntos: {len(trajectory)}\n"
                                   ...
                                   f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
                                   f"Tiempo entre pasos: {step_delay}s\n"
                                   f"Tiempo total estimado: {len(trajectory) * step_delay:.1f}s")
    ...
    def start_zigzag_microscopy(self):
        ...
        if not hasattr(self, 'current_trajectory'):
            QMessageBox.warning(self, "Error", "Primero debes generar una trayectoria zig-zag")
            return
    ...
    def execute_next_zigzag_point(self):
        """Ejecuta el siguiente punto de la trayectoria zig-zag."""
        ...
        logger.info(f"📍 Punto {self.zigzag_current + 1}/{len(self.zigzag_points)}: Moviendo a ({x_target:.1f}, {y_target:.1f}) µm")
