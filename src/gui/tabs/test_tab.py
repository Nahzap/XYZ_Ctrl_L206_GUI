...
class TestTab(QWidget):
    ...
    def __init__(self, trajectory_generator=None, parent=None):
        ...
        self._setup_ui()
        logger.debug("TestTab inicializado")
    ...
    def set_hardware_callbacks(self, send_command, get_sensor_value, get_mode_label):
        ...
        logger.debug("Callbacks de hardware configurados en TestTab")
    ...
    def _create_calibration_section(self):
        ...
        info = QLabel("<b>ℹ️ La calibración se configura en 'Análisis'</b>")
        info.setStyleSheet("padding: 8px; background: #34495E; border-radius: 5px;")
        layout.addWidget(info)
        
        self.calibration_status = QLabel("⚪ Sin calibración")
        ...
        self.calibration_details.setMaximumHeight(60)
    ...
    def _create_zigzag_section(self):
        ...
        self.tolerance_input = QLineEdit("35")
        self.tolerance_input.setFixedWidth(80)
        self.tolerance_input.setToolTip("Error máximo permitido para considerar que llegó al punto (~35µm recomendado)")
    ...
    def set_calibration(self, calibrated: bool, details: str = ""):
        """Actualiza estado de calibración."""
        if calibrated:
    ...
    def set_controller(self, motor: str, controller_data: dict):
        ...
        if motor == 'A':
            ...
            logger.info(f"Controlador A guardado en TestTab")
        else:
            self.controller_b = controller_data
            info = f"Kp={controller_data['Kp']:.4f}, Ki={controller_data['Ki']:.4f}\n"
            info += f"γ={controller_data.get('gamma', 0):.4f}, U_max={controller_data.get('U_max', 100):.1f}"
            self.set_controller_b(info, True)
            logger.info(f"Controlador B guardado en TestTab")
    ...
    # ============================================================
    # CONTROL DUAL EN TIEMPO REAL (usando callbacks)
    # ============================================================
    
    def start_dual_control(self):
        """Inicia control dual de ambos motores usando callbacks."""
        logger.info("=== INICIANDO CONTROL DUAL ===")
        
        # Verificar callbacks
        if not self.send_command_callback:
            QMessageBox.warning(self.parent_gui, "Error", "Callbacks de hardware no configurados")
            logger.error("Callbacks no configurados")
            return
        
        # Verificar que al menos un controlador esté cargado
        if not self.controller_a and not self.controller_b:
            QMessageBox.warning(self.parent_gui, "Error",
                              "No hay controladores cargados.\n\nTransfiere desde 'H∞ Synthesis'.")
            logger.error("No hay controladores cargados")
            return
        
        # Obtener referencias
        try:
            self.ref_a_um = float(self.ref_a_input.text()) if self.controller_a else 0
            self.ref_b_um = float(self.ref_b_input.text()) if self.controller_b else 0
            logger.info(f"Referencias: A={self.ref_a_um}µm, B={self.ref_b_um}µm")
        except:
            QMessageBox.warning(self.parent_gui, "Error", "Referencias inválidas")
            return
        
        # Activar modo automático en Arduino
        ...
        self.dual_integral_a = 0.0
        self.dual_integral_b = 0.0
        self.dual_last_time = time.time()
        self._position_reached = False  # Resetear flag de posición alcanzada
        ...
        # Crear timer
        self.dual_control_timer = QTimer()
        self.dual_control_timer.timeout.connect(self.execute_dual_control)
        self.dual_control_timer.start(10)  # 100Hz
        
        self.results_text.append("🎮 Control Dual ACTIVO")
    ...
    def execute_dual_control(self):
        ...
        Cuando AMBOS motores llegan a su posición objetivo (error < 35µm):
        - Detiene los motores con freno activo
        - Muestra mensaje de posición alcanzada
        - Mantiene el control activo para correcciones menores
        """
        try:
            # Calcular Ts
            ...
            # Control Motor A (usa Sensor 2 según análisis)
            if self.controller_a:
                ...
                if sensor_adc_a is not None:
                    # Convertir referencia µm a ADC usando calibración
                    ref_adc_a = (21601.0 - self.ref_a_um) / 12.22
                    ref_adc_a = max(0, min(1023, ref_adc_a))
                    
                    # Error en ADC y µm
                    error_a = ref_adc_a - sensor_adc_a
                    error_a_um = error_a * 12.22  # Para verificación de posición
            ...
            if self.controller_b:
                ...
                if sensor_adc_b is not None:
                    ref_adc_b = (21601.0 - self.ref_b_um) / 12.22
                    ref_adc_b = max(0, min(1023, ref_adc_b))
                    
                    error_b = ref_adc_b - sensor_adc_b
                    error_b_um = error_b * 12.22
            ...
            # Verificar si AMBOS motores llegaron a la posición (tolerancia 35µm)
            TOLERANCE_UM = 35.0
            a_at_target = abs(error_a_um) < TOLERANCE_UM if self.controller_a else True
            b_at_target = abs(error_b_um) < TOLERANCE_UM if self.controller_b else True
            both_at_target = a_at_target and b_at_target
            
            # Inicializar flag de posición alcanzada si no existe
            if not hasattr(self, '_position_reached'):
                self._position_reached = False
            
            if both_at_target and not self._position_reached:
                # ¡POSICIÓN ALCANZADA! Detener motores
                ...
                self.results_text.append(
                    f"✅ POSICIÓN ALCANZADA: A={self.ref_a_um:.0f}µm (err={error_a_um:.1f}), "
                    f"B={self.ref_b_um:.0f}µm (err={error_b_um:.1f}) - Motores bloqueados"
                )
                logger.info(f"✅ Posición alcanzada - Motores bloqueados")
                return  # No enviar más comandos
            
            elif not both_at_target:
                # Si se salió de la posición, reactivar control
            ...
            # Si ya está en posición, no enviar comandos (mantener bloqueado)
            
            # Log cada 50 ciclos (reducido para menos spam)
            if not hasattr(self, '_dual_log_counter'):
                self._dual_log_counter = 0
            self._dual_log_counter += 1
            if self._dual_log_counter % 50 == 0:
                status = "✅" if both_at_target else "🔄"
                self.results_text.append(
                    f"{status} A: {error_a_um:.1f}µm | B: {error_b_um:.1f}µm | PWM: ({pwm_a},{pwm_b})"
                )
    ...
    def stop_dual_control(self):
        ...
        # Detener timer PRIMERO para evitar más comandos
        if self.dual_control_timer:
            self.dual_control_timer.stop()
            self.dual_control_timer = None
        
        # FRENO ACTIVO INMEDIATO - Comando B para cortocircuitar motores
        if self.send_command_callback:
            self.send_command_callback('B')  # Freno activo Arduino v0.2
            time.sleep(0.1)  # Esperar que el freno actúe
            
            # Luego volver a modo manual
            self.send_command_callback('A,0,0')
        ...
        self.results_text.append("⏹️ Control Dual DETENIDO (Freno Activo)")
        logger.info("Control dual detenido con freno activo")
    ...
    def start_trajectory_execution(self):
        """Inicia la ejecución de la trayectoria zig-zag."""
        logger.info("=== INICIANDO EJECUCIÓN DE TRAYECTORIA ===")
        ...
        if not self.send_command_callback:
            self.results_text.append("❌ Error: Callbacks no configurados")
            return
        
        # Verificar que hay al menos un controlador
        if not self.controller_a and not self.controller_b:
            self.results_text.append("❌ Error: Necesitas al menos un controlador")
            return
        
        # Obtener parámetros
        try:
            self.trajectory_tolerance = float(self.tolerance_input.text())
            self.trajectory_pause = float(self.pause_input.text())
        except:
            self.trajectory_tolerance = 100
        ...
        self.trajectory_timer.start(10)  # 100Hz
        
        # Actualizar UI
        self.set_zigzag_active(True)
        self.results_text.append(f"🚀 Ejecutando trayectoria: {len(self.current_trajectory)} puntos")
        self.results_text.append(f"   Tolerancia: {self.trajectory_tolerance}µm, Pausa: {self.trajectory_pause}s")
        
        logger.info(f"Trayectoria iniciada: {len(self.current_trajectory)} puntos")
    ...
    def execute_trajectory_step(self):
        ...
        2. Cuando llega al punto → DETIENE motores (freno), inicia pausa
        3. Durante la pausa → NO envía comandos, motores quietos
        4. Después de la pausa → Siguiente punto, reinicia control
        """
        try:
            ...
            # Convertir a ADC (calibración: pendiente=-12.22 µm/ADC, intercepto=21601 µm)
            ref_adc_x = (21601.0 - target_x) / 12.22
            ref_adc_y = (21601.0 - target_y) / 12.22
            ref_adc_x = int(max(0, min(1023, ref_adc_x)))
            ref_adc_y = int(max(0, min(1023, ref_adc_y)))
            ...
            # Control Motor A (eje X, sensor 2)
            if self.controller_a:
                sensor_key = 'sensor_2' if self.motor_a_sensor2.isChecked() else 'sensor_1'
                ...
                if sensor_adc is not None:
                    ...
                    error_x_um = error_adc * 12.22  # Convertir a µm para verificación
            ...
            # Control Motor B (eje Y, sensor 1)
            if self.controller_b:
                sensor_key = 'sensor_1' if self.motor_b_sensor1.isChecked() else 'sensor_2'
                sensor_adc = self.get_sensor_value_callback(sensor_key)
                ...
                if sensor_adc is not None:
                    ...
                    error_y_um = error_adc * 12.22  # Convertir a µm para verificación
            ...
            # Verificar si llegamos al punto (tolerancia en µm)
            at_target = abs(error_x_um) < self.trajectory_tolerance and abs(error_y_um) < self.trajectory_tolerance
            
            if at_target:
                # ¡LLEGAMOS AL PUNTO! 
                # PASO CRÍTICO: DETENER MOTORES para que queden quietos durante la pausa
                self.send_command_callback('B')  # Freno activo
                time.sleep(0.05)
                ...
                self.results_text.append(
                    ...
                    f"[Error: X={error_x_um:.1f}, Y={error_y_um:.1f}µm] "
                    f"- Pausa {self.trajectory_pause}s"
                )
                logger.info(f"✅ Punto {self.trajectory_index + 1} alcanzado - Motores detenidos - Pausa {self.trajectory_pause}s")
            else:
                # Aún en movimiento - enviar comando de control
                self.send_command_callback(f"A,{pwm_a},{pwm_b}")
    ...
    def stop_trajectory_execution(self):
        ...
        logger.info("=== DETENIENDO EJECUCIÓN DE TRAYECTORIA ===")
        
        self.trajectory_active = False
        
        # Detener timer PRIMERO
        if hasattr(self, 'trajectory_timer') and self.trajectory_timer:
            self.trajectory_timer.stop()
            self.trajectory_timer = None
        
        # FRENO ACTIVO INMEDIATO
        if self.send_command_callback:
            self.send_command_callback('B')  # Freno activo
            time.sleep(0.1)
            self.send_command_callback('A,0,0')
        ...
        points_done = self.trajectory_index + 1 if self.current_trajectory else 0
        total_points = len(self.current_trajectory) if self.current_trajectory else 0
        self.results_text.append(f"⏹️ Trayectoria detenida en punto {points_done}/{total_points} (Freno Activo)")
        
        logger.info("Ejecución de trayectoria detenida con freno activo")
